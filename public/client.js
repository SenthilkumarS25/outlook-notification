...
socket.on('notification_received', notificationData => {
  ...
  if (notificationData.sender) {
    ...
    primaryText.innerText = `Sender: ${notificationData.sender.emailAddress.name}`;
    listItem.appendChild(primaryText);
    listItem.appendChild(document.createElement('br'));
  }

  const secondaryText = document.createElement('span');
  secondaryText.innerText = `Subject: ${notificationData.subject ? notificationData.subject : JSON.stringify(notificationData)}`;
